document.addEventListener("DOMContentLoaded", function () {
    const apiUrl = "http://13.200.156.231:8097/maaBaglamukhi/getAll/maaBaglamukhi";
    let row = '';
        ...
        .then(response => response.json())
            .then(data => {
                ...
                userdata.map((item, index) => {
                    ...
                    row += `<div class="card">
                        <img src="http://13.200.156.231:8097/maaBaglamukhi/displayHotelImage?id=${item.id}" alt="" class="ujjainimage">
          ...
          <div><p class="toprate"> ₹ ${item.price}<span class="btn1"> <div class="btn"><button class="Booknow" >Book Now</button></div></span></p></div>
                ...
                document.getElementById('maa').innerHTML = row;
            })
            .catch(error => {
        ...
        .then(response => response.json())
            .then(data => {
                ...
                userdata.map((item, index) => {
                    //console.log(item.email)
                    row += `<div class="card1">
          <img src="http://13.200.156.231:8097/maaBaglamukhi/displayHotelImage?id=${item.id}" alt="" class="ujjainimg1">
          ...
          <div><p class="toprate1"> ₹ ${item.price}<span class="btn1"> <div class="btn1"><button class="Booknow1" >Book Now</button></div></span></p></div>
                ...
                document.getElementById('maa1').innerHTML = row;
            })
            .catch(error => {
                console.error("Error fetching data:", error);
            });
